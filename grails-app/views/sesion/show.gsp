...
<html>
  ...
  <body>
    ...
    <div id="list-prestamos" class="content scaffold-list" role="main">
      <h1>Prestamos por sesión de: <g:fieldValue disabled="true" bean= "${sesionInstance?.user}" field="name"/> 
        @ <g:formatDate date="${sesionInstance?.fecha}" formatName = "formato.fecha" /></h1>
      ...
      <table>
        ...
        <tbody>
          <g:each in="${lab.Prestamo.findBySesion(sesionInstance)}" status="i" var="prestamo">
            <tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
              ...
              <!-- Aval Boleta-->
              <td>
                <g:link action="show" controller = "prestamo" id="${prestamo.id}">
